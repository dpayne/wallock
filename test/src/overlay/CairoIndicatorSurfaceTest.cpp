...
class CairoIndicatorSurfaceMock : public wall::CairoIndicatorSurface {
   public:
    ...
    auto get_buffer_size(int32_t buffer_diameter, int32_t scale) -> std::pair<int32_t, int32_t> override {
        return wall::CairoIndicatorSurface::get_buffer_size(buffer_diameter, scale);
    }
...
TEST(CairoIndicatorSurfaceTest, test_should_draw) {
    ...
    indicator_surface.draw(1920, 1080, 1);
...
TEST(CairoIndicatorSurfaceTest, buffer_size) {
    ...
    auto [width, height] = indicator_surface.get_buffer_size(buffer_diameter, 1.0);
    ...
    keyboard.m_is_caps_lock = true;
    indicator_surface.update_message();
    auto [width2, height2] = indicator_surface.get_buffer_size(buffer_diameter, 1.0);
